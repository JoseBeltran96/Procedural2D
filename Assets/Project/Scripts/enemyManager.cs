using System.Collections;
using System.Collections.Generic;
using UnityEngine;

public class enemyManager : MonoBehaviour
{
    public float velocidad = 3f; // velocidad de movimiento
    public float tiempoDeMovimiento = 3f; // tiempo de movimiento
    public float fuerzaSalto = 5f; // fuerza del salto
    public LayerMask capaObstaculo; // capa de los obstáculos
    public LayerMask Ground;
    public bool isGrounded = false;

    private Rigidbody2D rb;
    private float tiempoRestante;
    private bool mirandoDerecha = true;

    //Caja debajo del personaje
    public float boxPositionX = 0f;
    public float boxPositionY = 0f;
    public float boxSizeX = 10f;
    public float boxSizeY = 10f;

    private void Start()
    {
        rb = GetComponent<Rigidbody2D>();
        tiempoRestante = tiempoDeMovimiento;
    }

    private void Update()
    {
        isGrounded = p_groundedCheck();
        
    }

    private void FixedUpdate()
    {
        // Movimiento horizontal
        if (mirandoDerecha)
        {
            rb.velocity = new Vector2(velocidad, rb.velocity.y);
        }
        else
        {
            rb.velocity = new Vector2(-velocidad, rb.velocity.y);
        }

        // Detectar obstáculos delante
        Vector2 direccion = mirandoDerecha ? Vector2.right : Vector2.left;
        RaycastHit2D hit = Physics2D.Raycast(new Vector2(transform.position.x, transform.position.y - 0.1f), direccion, 1f, capaObstaculo);
        //Debug.DrawRay(new Vector2(transform.position.x, transform.position.y - 0.1f), direccion * 1, Color.red, capaObstaculo);

        if (hit.collider != null && isGrounded == true)
        {
            // Si se detecta un obstáculo y no está por debajo del enemigo
            rb.AddForce(Vector2.up * fuerzaSalto, ForceMode2D.Impulse);
        }

        // Cambiar dirección al acabar el tiempo de movimiento
        tiempoRestante -= Time.deltaTime;
        if (tiempoRestante <= 0)
        {
            mirandoDerecha = !mirandoDerecha;
            transform.localScale = new Vector3(-transform.localScale.x, transform.localScale.y, transform.localScale.z);
            tiempoRestante = tiempoDeMovimiento;
        }
    }

    //Comprueba si el personaje esta en el suelo
    private bool p_groundedCheck()
    {

        BoxCast(new Vector2(transform.position.x - boxPositionX, transform.position.y - boxPositionY), new Vector2(boxSizeX, boxSizeY), 0f, new Vector2(0, 0), 0f, Ground);

        bool p_floorCollider;
        p_floorCollider = Physics2D.BoxCast(new Vector2(transform.position.x - boxPositionX, transform.position.y - boxPositionY), new Vector2(boxSizeX, boxSizeY), 0f, new Vector2(0, 0), 0f, Ground);

        return p_floorCollider;
    }

    static public RaycastHit2D BoxCast(Vector2 origen, Vector2 size, float angle, Vector2 direction, float distance, int mask)
    {
        RaycastHit2D hit = Physics2D.BoxCast(origen, size, angle, direction, distance, mask);

        //Setting up the points to draw the cast
        Vector2 p1, p2, p3, p4, p5, p6, p7, p8;
        float w = size.x * 0.5f;
        float h = size.y * 0.5f;
        p1 = new Vector2(-w, h);
        p2 = new Vector2(w, h);
        p3 = new Vector2(w, -h);
        p4 = new Vector2(-w, -h);

        Quaternion q = Quaternion.AngleAxis(angle, new Vector3(0, 0, 1));
        p1 = q * p1;
        p2 = q * p2;
        p3 = q * p3;
        p4 = q * p4;

        p1 += origen;
        p2 += origen;
        p3 += origen;
        p4 += origen;

        Vector2 realDistance = direction.normalized * distance;
        p5 = p1 + realDistance;
        p6 = p2 + realDistance;
        p7 = p3 + realDistance;
        p8 = p4 + realDistance;


        //Drawing the cast
        Color castColor = hit ? Color.red : Color.blue;
        Debug.DrawLine(p1, p2, castColor);
        Debug.DrawLine(p2, p3, castColor);
        Debug.DrawLine(p3, p4, castColor);
        Debug.DrawLine(p4, p1, castColor);

        Debug.DrawLine(p5, p6, castColor);
        Debug.DrawLine(p6, p7, castColor);
        Debug.DrawLine(p7, p8, castColor);
        Debug.DrawLine(p8, p5, castColor);

        Debug.DrawLine(p1, p5, Color.grey);
        Debug.DrawLine(p2, p6, Color.grey);
        Debug.DrawLine(p3, p7, Color.grey);
        Debug.DrawLine(p4, p8, Color.grey);
        if (hit)
        {
            Debug.DrawLine(hit.point, hit.point + hit.normal.normalized * 0.2f, Color.yellow);
        }

        return hit;
    }
}
